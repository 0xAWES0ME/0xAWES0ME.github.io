...
## Update: Even better
Michael Huzman pointed out that the builtin `popcnt` instruction should vastly improve the speed of the `inner` function. By adding the flag `-march=corei7` gcc will automatically use this instruction for bitset's `count` function. This again halves the speed and now it runs in a single second!
